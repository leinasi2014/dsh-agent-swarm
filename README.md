...
- request/retry/deadline 预算，以及从 DSH `assistant/message.usage` 按 event seq 去重的完整计费 token（uncached input、output、cache read/write）计量；连续 usage 事件按 scope+session 微批合并为单次事务写（seq 游标幂等，replay/重载不双计）；预算生命周期与 workflow run 解耦（M2-5/#79）：同一 captain 的顺序 run 消费一条结转账本（`set_budget` 一次、多个 run 消费），run 边界的 usage 折返沿用同一游标幂等语义；
