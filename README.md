...
- 团队共享记忆与成员私有笔记保持独立授权和持久化边界。成员可新增、修订、作废或替代本人的笔记；Host 开启自动召回后，下一合法任务可读取相关有效经验，旧记录与操作历史保留。
...
当前依赖基线为官方 DSH `0.1.5-alpha.2`。Session 使用官方 V3 JSONL persistence；客户端通过 `remote.session.modelCatalog()` 读取模型目录。已有 Profile 升级前须保留原 Session/Storage，不把新空 Profile 的通过当成旧数据迁移验收。
...
个人经验自动召回默认关闭。在 Agent Swarm runtime 的 Host 配置中设置 `privateMemoryRecall: active-task` 后，仅在成员本人唯一有效的当前任务中召回有界经验；设为 `disabled` 撤下后续请求中的贡献。成员或队长无法通过提示词、工具许可或单次批准开启它。维护工具与权限细节见 [个人记忆合同](docs/04-core-protocol.md#71-个人记忆维护与召回合同)。
