...
- **成员配置与详情**：分离子智能体运行 Provider 和 LLM Provider/model，持久化创建时 deny 快照与 Skill 指派；成员默认延迟到首个任务指派才启动，避免无任务 join turn 与抢跑提交，旧 eager 行为仅由高级静态配置 `lazyMemberStart: false` 显式恢复；每任务动态工具改权在当前 DSH continuation seam 上明确为不支持。
