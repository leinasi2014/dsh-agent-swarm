...
function parseDocuments(registry) {
  ...
  for (const line of registry.split(/\r?\n/u)) {
    ...
    if (!current) continue
    const field = line.match(/^\s{4}(path|role|subject|writeMode|sourceAuthority|migrationState|mutationPolicy):\s*(.+?)\s*$/u)
...
for (const path of requiredLegacyEvidence) readRequired(path)
...
for (const document of documents) {
  if (!document.documentId || !document.path || !document.role || !document.subject || !document.writeMode) {
  ...
  if (document.role === 'stable-authority' || document.role === 'instruction-adapter') {
