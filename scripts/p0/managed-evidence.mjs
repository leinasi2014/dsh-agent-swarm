...
const IDENTITY = ['displayName', 'profession', 'personality', 'pixelAvatarSvg']
const TOOLS = {
  run_code: [],
  agent_swarm_create_managed: ['name', 'stage'],
  agent_swarm_set_captain_profile: ['expected_revision', 'display_name', 'profession', 'personality', 'pixel_avatar_svg'],
...
function checkSession(session) {
  ...
  keys(session.header, ['version', 'id', 'createdAt', 'parentSession', 'origin', 'seedLength', 'agentPreset'], 'Session header')
  assert(session.header.version === 0 && nonempty(session.header.id) && integer(session.header.createdAt), 'invalid official Session header')
  ...
  for (const event of session.events) {
    ...
    switch (event.type) {
      case 'user/message':
        ...
        keys(data.source, ['kind'], 'user source')
        assert(data.source.kind === 'user' && digest(data.contentSha256), 'direct user message identity missing')
        break
      case 'request/header':
        keys(data, ['header'], 'request header projection')
        keys(data.header, ['config'], 'request header')
        keys(data.header.config, ['provider', 'model'], 'request route')
      ...
      case 'tool/code-dispatch':
        keys(data, ['rootCallId', 'parentCallId', 'subCallId', 'name', 'arguments', ...(event.type === 'tool/code-dispatch' ? ['isError', 'content'] : [])], 'Code Mode dispatch')
        ...
        if (event.type === 'tool/code-dispatch') {
          assert(typeof data.isError === 'boolean' && Array.isArray(data.content) && data.content.length > 0, 'Code Mode result missing')
...
export async function verifyManagedEvidence(root, manifest, expected, failures) {
  try {
    ...
    assert(user?.type === 'user/message' && user.seq < main.call.seq, 'model-visible Main Brain user request missing')
    ...
    for (const [field, arg] of [['displayName', 'display_name'], ['profession', 'profession'], ['personality', 'personality'], ['pixelAvatarSvg', 'pixel_avatar_svg']]) {
      assert(nonempty(profileTeam.captainProfile?.[field]) && profileCall.args[arg] === profileTeam.captainProfile[field], 'Captain profile must be read back from exact update')
    }
    ...
    keys(observation, ['source', 'teamId', 'beforeRevision', 'afterRevision', 'captainProfile', 'members', 'tasks', 'screenshot'], 'UI readback')
    ...
    assert(screenshot.length > 8 && screenshot.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10])), 'UI supporting screenshot is not PNG')
