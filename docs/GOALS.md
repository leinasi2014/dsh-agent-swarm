...
**M4 — 能力族：预算与验证（2026-08-22 接替 M3）**

**入口门（开工前完成）**：#112 P0-2（sqlite 测试后端根治 .dsh-mkdir——命中率已 100%）；官方 0.1.1-rc.2 re-pin 累计审查（release 锚定下非紧急，与首个 M4 Gate A 合并做）；D2 前置条件 A（独立安全审查，场景 37）与 B（sandbox 硬围栏决定）在 M4 期间并行推进。

1. 预算能力族深化（官方 tokenMeter 适配评估 + 现有 Team 预算面的 M4 补全项——开工先做 Gate A + issue 分解）；
2. 验证能力族（独立可执行审查的泛化面：多验证根、验证命令库、与官方 invariants 面的对接评估）；
3. D2 前置条件 A/B 落地（审查 + 围栏决定）。

**D2 受控并行自托管已开放（2026-08-22 起，有条件）**：机制性前提满足（P0-P7 实证）；全面开放待前置条件 A/B——见 `docs/development/2026-08-22-m3-exit-report.md` §3。
