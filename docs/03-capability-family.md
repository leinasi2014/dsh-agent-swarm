...
本节约束 [选定产品方向](00-vision.md#32-团队目标与任务协作选定方向新增机制尚未实现)，不修改现行核心协议或声称新增 runtime 已可用。
...
2. **保留现有成员权限，显式增加人类入口。** 当前 `createTask` 与 self-claim 面向活跃 Team participant，指定别人和管理操作仍受 Captain 边界限制。本方案不把普通成员创建强制降为待审批提案。人类默认向 Captain 提出工作，Main Brain 同样通过带来源请求参与，不能借所查看成员的 Session 发起 mutation。未来允许人类直接发布任务或保存目标时，必须提供真实 Host 认证与写入合同；当前 Captain-only 的公共目标入口不能由 UI 直接冒充调用。
3. **开放认领是独立的任务策略扩展。** 拟保留 `automatic` 为默认，增加 `open-claim`；既有固定目标成员仅与 automatic 组合，open-claim 不与固定目标并存。开放任务从自动 Provider 候选中排除，Host 的自领与改派入口也核验同一策略。Captain 改为指派时，以同一任务 revision 原子更新策略与目标，不能只改界面标签。认领仍沿用依赖、active membership、成员忙碌、预算、revision CAS 和 attempt fencing。
...
5. **目标修订协调只由一个编排 owner 驱动。** 沿用 Team 的目标权威，拟记录目标修订及 Captain 已协调到的修订；目标变更、任务结果、阻塞和恢复等事件触发有界协调，任意 Team revision 变化不能反复唤醒自身规划。复用 adaptive/workflow 的既有 owner、邮箱和预算，不能让新的群聊循环与 workflow-run 同时驱动同一 Team。目标文字更新不改写已有任务、attempt 或历史输入，Captain 对受影响工作执行明确的保留、停止或替换。
6. **暂停在所有新增 attempt 边界一致执行。** 拟新增的推进控制同时覆盖自动派工、自领、retry 和重派产生新 attempt 的入口。暂停时保留已有 reserved/排队/运行 attempt 的投递和收尾，以及结果提交、审核和控制消息；它不是取消当前执行，也不是 archive。UI 可显示“暂停新增工作；仍有 N 项已分配/执行中”，不能只停一个前端按钮后宣称全队暂停。

有限目标由 Captain 对照完成标准确认；空任务板不自动表示目标达成。持续维护需耐久保存触发条件、下一行动、预算与恢复位置，一轮任务结束进入待命；没有配置触发条件时不标记长期运行已启动。目标写入、协调到新目标、任务投递及模型实际消费分别确认，UI 的“目标已更新，队长正在调整任务”必须有对应事实；原型只展示示例状态。
