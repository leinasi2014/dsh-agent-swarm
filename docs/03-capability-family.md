...
| 目的与范围 | 保存员工、任职、岗位记忆、Skills 提炼及审计的待决设计；部门与资料室规范见 [重构文档 01](refactoring/01-departments.md) |
...
部门职责、配置和行为政策的唯一正文已迁至[部门架构与资料室](refactoring/01-departments.md)。旧 ID 保留用于历史引用，不再维护两份规则：**DEP-D01** 对应 DEPT-01/02；**DEP-D02** 对应 DEPT-03。员工性格初始化、修改权与异议仍为 DEP-O02，不能因部门技术设计定稿而视为已决定。

### 10.4 身份、数据归属与执行接入（迁移入口）

**DEP-D03** 的部门、项目、Team、Session 和资料归属正文由[部门架构与资料室](refactoring/01-departments.md)的 DEPT-04、LIB-01/02 接替。任务、attempt、邮箱与审核保留既有权威；部门不建立第二套状态机。长期员工与任职的实际持久化、管理员替换时合法会话接管和跨域恢复仍属 DEP-O06，不能改写历史 Session parent，也不能仅改显示绑定就宣称接管成功。
...
| 阅读契约 | 内容 |
|---|---|
| 身份 / 范围 | 本文公司重构候选；沿用 §10 需求 ID，部门与资料室唯一正文仍是 [01-departments](refactoring/01-departments.md) |
| 成熟度 | 〔建议〕架构咨询及三模型反驳审查已收敛，以下包含统筹裁决；可以据此开展首片工程验证，不是产品完成或发布记录 |
| 证据 | 下列合同均未完成真实组合、并发、迁移与重启验收；模型接受方案不是技术验证 |
| 冲突规则 | 用户需求不降级；本节细化 §10 建议及待决项，未涉及部分沿用原文；不静默改变独立 Team 的现有运行合同 |
...
所有产生或替换 attempt 的 TeamDomainPort 及 runtime/工具/调度入口必须接入同一前门，首片验收的遗漏清单为空；不能只包装新公司 UI。已绑定员工的其他 Team/Session 也不得绕过，公司之外未绑定的旧成员维持原合同。组织临界区内按固定顺序读取权威，或使用在门内重建/校验的投影；若用水位，必须在提交前证明覆盖全部相关占用变更。无需强制非事务快照，更不能靠提交后发现冲突来撤销已返回成功的旧事实。
...
**ORG-11：旧 Team 显式纳管。** 独立 Team 原样支持，不因缺组织字段禁用；公司模式也不默认其成员 ready。预览部门/员工/任职/Session 映射、缺项、冲突、在途工作和不可逆约束，再由用户决定纳管。按稳定 operationId、schemaVersion、原/目的身份及检查点恢复；准备映射不授正式工作权，切换先收敛旧控制再提交唯一模式归属。未知结果查询权威，禁止新旧双控；不凭同名合并员工。

回退先暂停新工作、核对既有提交和资产变化；尚无不可逆行为可撤销未激活映射。撤权、删除或新模式执行后，不能删字段绕过准入，只能按当前授权重新接线或保持只读恢复。备份不代表可复活旧权。保留用户数据，先在隔离候选环境演练。

### 11.8 反例验收与技术问题

**ORG-12：先审查，再纵向交付。** 分期见[实施路线](07-implementation-roadmap.md)。三模型分工：Qwen 反驳产品/入职/成长缺口；GLM 检查身份/并发/接管/恢复；DeepSeek-Flash 独立检查跨域闭环、过度设计并综合结论。审查只读，不改生产配置、私有档案或实现代码。

| ID | 预期可观察结果（均未执行） |
|---|---|
| ORG-A01 | 空公司→部门→管理员招募→完整入职→真实领取/交付/审核→重启；各必要缺项在所有正式入口拒绝且可修复，个人文档版本/摘要与实际装配证据可重建 |
| ORG-A02 | 两 Team 竞争、撤权竞争、提交后回包/索引失败：只有合法 attempt，operationId 可查，未知时不重复领取 |
...
| ORG-A08 | 独立 Team 保留，纳管故障/重启/回退无双写或撤权复活；混合资产未定不假报彻底删除 |
