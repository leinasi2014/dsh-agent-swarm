...
- **A3** installs the official `agent-swarm` Settings namespace, contributes its official Plugins configuration card, and adds member/memory read UI with DSH locale/theme behavior. Its exit proof uses populated member/Team-memory/personal-memory data and Settings save/read-back across page and Profile reloads; an empty Team is not an A3 acceptance fixture.
