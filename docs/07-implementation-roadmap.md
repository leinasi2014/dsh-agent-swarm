...
当前官方包并未提供上述组合能力：TeamService 直接创建私有 Journal/Roster/Mailbox/TaskBoard，Journal 与若干公开读取依赖同步 Session projection。故推荐路线需要先完成官方 Team 模块的通用扩展与组合验证，不能以改服务名、强制类型转换、访问私有字段或假 Agent 代替。优先准备可上游维护的扩展；如需临时验证候选，必须固定源码、包及补丁版本，标明非官方发布物，后续回归上游后删除补丁。

| 必需扩展 | 最小职责与证据 | 不能省略的条件 |
|---|---|---|
...
具体接口签名在第一验证切片确定，上表不是声称已有同名官方 API。若必须更改官方 Task 状态或事件格式，明确版本和旧日志读取规则，不能静默向 strict schema 塞扩展字段。raw 官方工具和 Remote 也必须服从同一 Host 策略；仅从模型工具列表隐藏 `complete` 不能防止绕过 Review Gate。
...
| 阶段 | 具体产物 | 进入下一步的条件 |
|---|---|---|
| A：冻结功能合同 | 当前基线与上述能力逐项映射，保留既有回归，列出真实已知失败；确定官方发布版本和扩展差异 | 身份、事务、消息三个缺口都有可验证设计，不靠降低原要求消除差异 |
| B：验证官方可组合性 | 一个新建小团队在隔离 Profile 中使用官方候选组件、真实父子关系与现有 Storage Domain；只为该例补最小扩展 | 能创建/恢复成员，原子完成“移除成员 + 处理 attempt/预算 + 撤销旧消息”；缺服务、崩溃及 raw API 绕过反例通过 |
...
| E：复用任务通用内核 | 先公开并调用官方图校验；基本 CRUD/CAS 仅在官方组件能表达原状态和共同策略时继续接管；Swarm 的业务 mutation 与 attempt/审核/预算仍由同一事务执行 | 抢占、晚提交、审核旁路、重派、批量工作请求、目标维护和冷恢复全部通过；无等价替代的产品逻辑继续保留 |
...
阶段 B 必须先验证最强的共同事务，不先花大量时间重写 roster 然后才发现任务/邮箱无法协同。优先直接调用官方公开组件；需要公开现有通用内部组件时，只增加所需导出及合同，禁止复制官方源码到 Swarm 后称为采用。阶段 C–E 可以逐职责替换内部实现，但同一 Team 不并行运行两个写 owner；跨职责的事务始终由同一个后端执行。
...
采用的首要收益是假如官方组件能承接同一职责，就减少重复实现、重复修复及版本跟进的工作；这仍需计入官方扩展、适配层、迁移及后续升级成本。维护的总负担没有下降时，不能仅凭 Swarm 删除的行数宣称优化。当前还需要补三个组合缺口，直接整体替换会损失现有行为；先用阶段 B 判断这条路线是否值得继续。
...
| 恢复与维护成本 | 故障后恢复时间、重复/丢失/陈旧操作、历史可读性，以及一次官方升级演练需要改动的适配与补丁；迁移期间的临时成本单列 |
...
本轮先交付该方案和优先开发跟踪；采用实现、官方补丁发布和现有课堂切换均未发生。方案确定后优先推进采用切片，个人记忆、独立 Skills 和群聊 UI 的已授权目标继续保留，群聊视觉改动仍沿用户要求先方案后实现。
...
官方 Agent Teams 完整功能采用按 §2.1 优先评审和验证；下列其他切片保留各自范围和验收，后续按采用结果更新受影响实现，避免在即将替换的边界上重复开发。当前未合入候选及已知失败继续保留，不因优先级变化记成已完成。
