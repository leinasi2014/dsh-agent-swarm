...
### 记录与决策层（已接受证据保持不可变）
- [adr/](adr/) — 架构决策（ADR-0001..0010）；每份 ADR 自身的 Status 决定其为 proposed 或 accepted。Proposed ADR 是受审参考，不得单独成为生产实现或无关里程碑的强制门。当前核心重构候选由 [ADR-0010](adr/0010-model-autonomy-and-parked-attempts.md) 与 [Team runtime architecture blueprint v1](development/2026-08-24-team-runtime-architecture-blueprint-v1.md) 共同定义，须先经唯一非作者 QA。
