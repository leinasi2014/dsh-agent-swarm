...
| dependency lifecycle scripts (F1a) | repo-source installs in the freeze worktree and acceptance verification root run `pnpm install --frozen-lockfile --ignore-scripts`; `pnpm pack` runs under `npm_config_ignore_scripts=true` (pack rejects the flag itself) so candidate prepack/postpack cannot execute with PM authority. This is distinct from Profile tarball installation: the published package has no `postinstall`, and every Profile `plugin add -w <tarball>` uses normal lifecycle scripts without `--ignore-scripts`. |
