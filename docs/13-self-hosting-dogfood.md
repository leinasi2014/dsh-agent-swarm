...
真实使用按 [效率计量标准](08-testing-verification.md#团队效率计量标准) 采样，按 [通信交互约定](04-core-protocol.md#51-通信交互与效率约定) 报告实际阶段。优先提交新类型、反复出现、阻塞工作或明显浪费的代表性案例；同类无变化信息合并，不逐步刷屏。

每份反馈使用以下最小模板，无法取得的字段填写未知：

```text
场景／验收条件：
实例与版本／实际 provider、model、effort：
关联 Main、Team、Session、turn、work_request、task、attempt：
采样窗口、事件 seq、时钟精度、样本 N：
实际达到阶段／首次可用结果／最终审核与未完成部分：
elapsed 与可证明分段／模型步骤／顶层与 PTC 工具计数：
非缓存输入／缓存读、写／输出／官方规范化 total 及已知来源（缺失项为未报告）：
错误码、根因事件次数、恢复动作与耗时、最终是否提交成功：
上下文增长、重复事实或无效等待的证据：
结论、未知项、一个拟验证改善点／下一责任方：
采样是否增加模型调用或写入、其范围：
```

反馈给统筹者分类；实现者、独立检查者和统筹者分别报告自己实际完成的阶段。已恢复的调用错误保留恢复证据；新的阻塞、反复失败、结果不确定或可能影响数据正确性的问题及时上报。用户主动查看、主动改需求及需求澄清计入对应原因，不归成自动导航或插件故障。

统计结果是有界历史证据，保存在当前任务或既有证据位置；本文件不维护运行状态、任务队列、租约、逐次用量或告警清单。试行通信约定不代表尚未实现的工单更正、自动计量或调度功能已经交付。
