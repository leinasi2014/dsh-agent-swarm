...
6. Feed live Agent availability into Scheduler selection, delimit untrusted model-visible data and carry the accepted compatibility hardening. Partially done in M1C (#13): usage-event write coalescing landed with cursor-idempotent batches, and the F11/F12/F14/F15 companion set closed (see §1); live-status scheduling (F10) and prompt delimiting (F8) remain, and the quiet inactive-delivery compatibility half stays with the #19 official-compat group.
