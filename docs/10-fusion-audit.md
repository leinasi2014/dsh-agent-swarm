...
|---|---|---|
| deterministic SwarmFlow, parallel/pipeline/nested nodes | Partial | M2-1 (issue #75): the official abstract `WorkflowEngine` has a Team-backed Provider (`TeamBridgeWorkflowEngine`, isolated service scope, run overlay as the only run truth, real-composition tested with the official invariant companion); Jiuwen phase/parallel/pipeline mapping rides the official script realm. Not yet wired: `adaptive|workflow` mode selection (#77) and run-as-job disclosure/cancellation (#76) |
