...
It remains a partial synthesis. Jiuwen's workflow, Worktree, distributed control/data plane, human nodes, permission tiers, automatic Team memory, Skill Evolution and UI are absent. Several official/community robustness properties are also incomplete: unload is unbounded (F4, M1C), retained attempt history grows without a limit (F7, M1B), and scheduling ignores actual member availability (F10, M1C).
...
| Whole-aggregate protocol core | `src/domain/team-domain.ts` composing the roster/board/mailbox/budget/projection modules (600-line guardrail holds with zero exceptions) | M1B attempt-retention restructuring (F7) reworks the board/attempts path in place; the F6 mailbox rework landed in `team-domain-mailbox.ts` |
...
4. ~~Align limits with behavior: per-target pending mailbox quota, bounded retained receipts and bounded attempt history.~~ **Mailbox half closed in M1B (F6, 2026-08-20)**: `maxPendingMessagesPerMember` (official default 64) counts only queued-minus-delivered mail per target with the official `TEAM_MAILBOX_FULL` code, and retained delivered/cancelled receipts are bounded (`maxRetainedMessages`) with oldest-first pruning that never touches queued mail; pre-F6 schema-v1 records load unchanged. Bounded attempt history remains the separate F7 issue.
...
- Since M1A every transaction writes one complete Team aggregate record through the official domain write chain (the json backend republishes the whole unit file per write). Cost still grows with retained tasks/attempts/memory — the whole-aggregate write boundary is deliberate (one Team revision = one durable write) — but parsing/validation now happen once at domain open plus a semantic pass per read; mailbox retention is bounded since F6 (`maxPendingMessagesPerMember` admission, `maxRetainedMessages` receipts), and the unbounded attempt growth driver (F7) is scheduled M1B work. A typed incremental backend remains the answer before large Teams or multiple processes.
