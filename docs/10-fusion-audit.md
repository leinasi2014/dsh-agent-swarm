...
It remains a partial synthesis. Jiuwen's Worktree, distributed control/data plane, human nodes, permission tiers, automatic Team memory, Skill Evolution and UI are absent; its deterministic workflow direction now rides the official Workflow seam through the M2-1 Team bridge Provider (issue #75), with the M2-3 explicit orchestration modes and single-owner discipline landed (issue #77); run-as-job disclosure/cancellation (#76) remains.
...
|---|---|---|
| deterministic SwarmFlow, parallel/pipeline/nested nodes | Partial | M2-1 (issue #75): the official abstract `WorkflowEngine` has a Team-backed Provider (`TeamBridgeWorkflowEngine`, isolated service scope, run overlay as the only run truth, real-composition tested with the official invariant companion); Jiuwen phase/parallel/pipeline mapping rides the official script realm. M2-3 (issue #77) wired the `adaptive|workflow` mode selection with the single-owner discipline and dual-owner adversarial fencing tests. Not yet wired: run-as-job disclosure/cancellation (#76) |
| deterministic SwarmFlow, parallel/pipeline/nested nodes | Partial | M2-1 (issue #75): the official abstract `WorkflowEngine` has a Team-backed Provider (`TeamBridgeWorkflowEngine`, isolated service scope, run overlay as the only run truth, real-composition tested with the official invariant companion); Jiuwen phase/parallel/pipeline mapping rides the official script realm. M2-2 (issue #76) adds the read-only jobs-face projection of the same task board (`TeamJobProjection`, official `JobRegistry` implementation, isolated `jobs` scope, no projection store, dual-face consistency tested with the official jobs invariant companion). Not yet wired: `adaptive|workflow` mode selection (#77) and the model-facing job reader exposure |
