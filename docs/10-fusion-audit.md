...
| `ctx.workflowEngine` / `ctx.jobs` | workflow: Provider (M2-1); jobs: No | M2-1 (issue #75): the bridge implements the official `WorkflowEngine` abstract class and registers in an isolated `workflowEngine` scope (never over the default-scope official engine); run truth lives in the project-owned `agent_swarm_workflow` overlay domain because nested Team runs have no official durable record. `ctx.jobs` remains unconsumed until #76 |
