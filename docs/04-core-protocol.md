...
不添加顶部 Team 按钮；关闭后可从官方侧栏的新页签引导页重新打开 Team。现有 controller 随当前 Session 只读观察，首次载入、创建团队及重连后取得完整 Host 投影，且当前右栏已经展开时才自动申请页签，不显示无团队空卡。关闭、切到其他官方页签或收起右栏后，同一 Team 的刷新不得抢回焦点；加载期间关闭同样有效。官方 tab.id 仅在 Session 内唯一，插件按 Session 与 tab.id 跟踪绑定；组件卸载只表示正文不可见，只有 tab.signal abort 表示实际关闭。切换 Session 时保留各自页签；右栏隐藏时不因切换 Team、Captain 或成员而展开，用户仍可显式打开。新打开的队长/成员 Chat 优先选中 Host 验证的所属 Team；主会话默认选择未归档 Team，用户显式选中的其他或历史 Team 在当前 Session 内保留。轮询、请求取消和卸载仍由原 controller 生命周期负责。
