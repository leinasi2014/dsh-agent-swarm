...
Isolation is the **workspace scope + durable Team identity + member's durable Session identity** tuple. The table key is `JSON.stringify([scope, teamId, memberSessionId, seq])`; reads also filter on the authoritative record **fields** (scope/teamId/memberSessionId), so a tampered record that reuses a member's key with forged identity fields never surfaces in that member's view (locked by a store-level tamper test). `seq` is the member-local monotonic creation index; `memory_id = private-memory-<seq>`.

## Authorization (owning active member only)

`resolveOwningMember` in the runtime resolves `exec.agent`, derives the workspace scope, and calls the existing `domain.requireMembership(scope, agent.id)`, then narrows to `role === 'member'` (an active roster row, Phase `active`). Because the partition key is the caller's own durable Session id, a member can never address another member's private memory; there is no target parameter to attack.
...
- `apply` opens the domain via `ctx.storageDomain.open(privateMemoryDomainSpec)`, constructs `MemberPrivateMemoryStore`, attaches `runtime.privateMemory`.
...
- `tests/member-private-memory-store.spec.ts` (9 unit tests over the real storage-domain stack with an injected faultable medium): ordering/id monotonicity, pagination, **scope isolation with identical Team+member ids**, cross-member/cross-team isolation, evidence truncation, invalid input (`TEAM_INPUT_INVALID`/`TEAM_INPUT_LIMIT`), flush/append failure leaving no partial state, closed-store rejection, and a key-reuse tamper record never surfacing.
- `tests/member-private-memory-real-composition.spec.ts` (2 real dual-context tests): (1) member writes in Context A; Team snapshot deep-memory/content invariants + read-surface non-leakage; full dispose of A; B reopened over the SAME official SQLite + Storage Domain roots with only official `ctx.agents.resume` recovery; exact `list` restoration, further appends in B, pagination, peer/captain/outsider rejection, removed-member and archived-team rejection. (2) a precise invariant: after quiescing the Team revision, a **direct `runtime.addPrivateMemory` call with the exact live member exec** leaves the whole Team snapshot **deep-equal** (proving private storage never mutates the aggregate), and the real tool face still lands normal `tool/call`+`tool/result` on the member's own official Session.
