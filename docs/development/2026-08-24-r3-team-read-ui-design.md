...
R3 adds the first DSH-native browser surface for the accepted R2 read contract. Its presentation is an icon-only Session-header utility immediately after Session log that opens a plugin-owned, non-modal anchored Peek Card, not a replacement conversation view. On desktop the card is a rounded 400–420px right-aligned floating surface with viewport clearance; at widths up to 720px only its horizontal geometry expands while the Session header and Team trigger remain visible. It reads only the packed public `/swarm/v1` client and shows the exact Host-projected Team, roster, tasks, attempts, budget, pending interactions and capability ceiling.
...
- The card has no backdrop, focus trap or `aria-modal` claim. The same Team button toggles it; Escape and the close control restore focus to that trigger. An outside pointerdown closes without cancellation or focus restoration, so the original Chat interaction continues. The 720px responsive rule changes only horizontal geometry, not data ownership or interaction authority, and never covers the trigger.
