...
- The Details footer offers read-only Captain Chat handoff through the existing controller. It verifies that the projected owner root remains in the official `sessions.list` snapshot, then calls public `sessions.open(rootSessionId)`; an absent root fails closed. A separate Tool Details utility explicitly yields the Team priority lease and leaves the official Details column open for the selected official Tool renderer.
