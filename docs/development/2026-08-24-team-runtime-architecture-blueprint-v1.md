...
Accepted A1a base: repository commit `e72f0344191360aef56b9055da63cb88544ba64e`. A1b is frozen at `e436e38d44374b46861174f4d494595c3854bcc8` with isolated official Profile evidence and two non-author PASS verdicts; its claim remains `DEV_SMOKE_ONLY`, not release acceptance. The A2a online continuation sub-slice is accepted at `e15a3fd0aae3d8aed3f182c64a5a2f56eeadb5a4`. The A2a-R1a cold entered-dispatch slice is accepted at `1d27deb0fbe61df87a6f32a690df5beace239a14`: exact assistant or non-repair terminal evidence settles, while missing/repair-only/conflicting evidence becomes `dispatch-unknown` without retry. A2a-R1b is accepted at `8c18feaa1e4fea652bca30214836505b5cd436e0`: after hooks and the current witness are proven, it compares official physical Session state before and after exclusive cold repair and atomically stages one deterministic, proof-bound `kind=recovery, phase=frame-pending` reservation. A2a-R1c is accepted at `0b186cbe480c1bff9aecc4e1f7d43eb2aa02e082`: absent-only typed recovery delivery, atomic durable claim handoff, common Provider/assistant settlement and cold claimed-frame folding are proven without redelivery; a durably pending unclaimed frame remains fail-closed because official DSH exposes no public resume-only operation for that exact MessageId. The R1d candidate adds running exact-member submit and the fence/release phase of captain-only reassign: task revision and Attempt CAS win atomically over initial/continuation/recovery epochs, every open dispatch and applied receipt is superseded in the same Team transaction, and the prior member is interrupted only after the fence commits. A stale typed frame or a one-shot Agent Loop permit whose tuple disappeared is refused before Provider entry, while permit retirement leaves later requests usable. Successor generation scheduling/claim is not implemented by this slice and the released task may remain pending. The accepted initial-outcome parity slice closes exact Session folding and before-park crash recovery; explicit cancel/principal expansion and Provider request/result read-back remain later slices. The current DBG-024 compatibility candidate makes lazy first-assignment startup the legacy product default, retains explicit `false` only for eager compatibility, and rejects eager startup under fresh-v2. Its existing GLM-5.3 + DSV4F evidence used explicit `true`, so omitted-key Profile reproof and exact-candidate acceptance remain pending; arbitrary upgrades with `provisioning` members are not claimed. Official DSH `0.1.1-rc.2` / `b150a551b8d465e31e418e1b2eaf5e79bbb7d28e` is the untouched verification host. JiuwenSwarm is reviewed and pinned at `e99393616946aef6d3fb655b5735cb52bff7809f`; dsh-agent-teams is reviewed and pinned at `912aae5225d3d85fa841a1b0c8a5c77021876c25` / `0.1.13`. LoopX is a remote, non-dependency reference whose relevant control-plane and native-DSH surfaces were reviewed at `6aa2fb8a9fb97f0bfa6ee8b0ca6fabf6265bbe95`. Gate A passes for the locally registered pins; implementation candidates still require milestone-specific non-author acceptance.

DBG-024 status update (2026-08-26): the omitted-key product default is now proven on exact
candidate `8c6373350f2b0ba8162af423ed59e4c396571ab0` by a clean GLM-5.3 Captain +
`dsv4f-207 / DeepSeek-V4-Flash-0731` member Profile run. The assignment was the member's first
user frame, one submission succeeded, Captain review accepted it and Team retries remained
zero. This supersedes the pending reproof statement in the implementation snapshot above;
arbitrary upgrades with `provisioning` members remain outside the claim.
...
| Captain explicitly reassigns or review rejects for rework | Atomically fence/release the old attempt to pending, optionally pin eligibility, then let a separately admitted scheduler/claim create the fresh generation. |
...
stateDiagram-v2
  ...
  Parked --> Stale: explicit reassign fence / review-owned rework policy; task returns pending
  Stale --> Reserved: later scheduler/claim creates a fresh generation and new Attempt
...
- Existing Gate A passes against official DSH `b150a55…`, Jiuwen `e993936…` and dsh-agent-teams `912aae5…`. Any later affected pin/export/Profile movement reruns the project verifier before production code resumes.
...
- **A2a continuation:** accepted online candidate `e15a3fd`, accepted cold entered-dispatch candidate `1d27deb`, accepted recovery-reservation candidate `8c18fea` and accepted recovery-delivery candidate `0b186cb` cover typed same-Attempt intent, official followup/claim, model-entry witness, entered-outcome reconciliation, proof-bound recovery staging and absent-only recovery delivery. The R1d candidate makes running-only submit and the reassign fence/release phase win the same aggregate transaction over all open continuation/recovery authority, rejects stale frames/permits before Provider entry and retires stale permits without poisoning later requests. The initial-outcome parity candidate adds exact online/cold Session folding, no-output parking, partial/conflict unknown and the cold `running + settled` before-park repair with no Provider replay. Successor scheduling/claim, explicit cancel/principal expansion and Provider request/result read-back remain; no elapsed-time wake or manager approval is added.
