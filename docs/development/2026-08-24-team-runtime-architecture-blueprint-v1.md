...
- **A2a continuation:** accepted online candidate `e15a3fd` lets one owning member write one typed same-attempt continuation intent during its current official turn, park only after durable turn settlement, reserve one once-only receipt before effect, resume through Captain-owned official `followup` plus exact target inbox claim and the common Agent Loop dispatch witness, then settle from exact assistant evidence. Its composition gate proves duplicate/conflicting request, exact principal, same Attempt/generation, one effect/frame/dispatch and rejection of an ordinary unframed later wake. The following A2a-R1 entered-dispatch slice adds a fresh-composition fold over reopened official stores: an official unpublished Session reservation commits repair and excludes concurrent local resume while one Team transaction settles assistant/terminal evidence; missing/interrupted/conflicting evidence becomes durable `dispatch-unknown` without replay. This still is not the A2a exit: crash-before-effect, crash-after-admission in `frame-pending`, proven-not-entered `dispatch-pending -> recovery`, and submit/reassign/cancel competition must land and pass. No elapsed-time wake or manager approval is added.
