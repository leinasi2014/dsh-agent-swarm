...
- **A2a continuation:** one owning member parks the exact current Attempt, writes one typed same-attempt continuation intent and one once-only effect identity, resumes through the official continuation/Agent Loop path, and settles from exact target evidence. Prove duplicate request, principal race, process restart, crash-before-effect, crash-after-admission, dispatch-pending, dispatch-entered/unknown and submit/reassign/cancel competition. No elapsed-time wake or manager approval is added.
