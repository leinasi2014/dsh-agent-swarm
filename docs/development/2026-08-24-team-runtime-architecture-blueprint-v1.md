...
- **A2a continuation:** accepted online candidate `e15a3fd` and accepted cold entered-dispatch candidate `1d27deb` cover typed same-Attempt intent, official followup/claim, model-entry witness and entered-outcome reconciliation. A2a-R1b next stages one proof-bound recovery reservation for an exact cold `dispatch-pending` epoch while leaving the old epoch current and producing no external effect. This still is not the A2a exit: recovery-trigger delivery/claim, cold `frame-pending`, crash-before-effect, and submit/reassign/cancel competition must land and pass. No elapsed-time wake or manager approval is added.
