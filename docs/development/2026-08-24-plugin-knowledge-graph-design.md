...
| Model tools | all 20 names (19 default + one config-gated fresh-v2 continuation tool), schema/register order, caller role, domain/effect, output/bounds | `src/tools.ts`, `src/tools/**`, `src/runtime/permission-policy.ts` | tool tests + composition |
...
- Populate every `src` module, all 20 tools (including conditionally registered tools with truthful availability), package/public exports, TeamDomain public methods/entities/enums, Config/defaults, inject/provide/get, Provider registries, Domains, RPC, Client slots/surfaces/Settings, events/listeners and lifecycle resources.
