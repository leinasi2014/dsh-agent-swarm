...
| Official host-tool inheritance and child report/code transport | current main retains its accepted permission boundary but does not contain the full legacy delta | legacy changes unlisted host tools from blanket default-deny to inherited official downstream authority, restores `run_code` transport, and adds child-scoped official `report` host-plane guards/composition tests | Truly new official-compatibility behavior with security consequences; neither a simple fixture nor safe wholesale port | `REIMPLEMENT` in a dedicated compatibility/permission pipeline after current installed DSH exports and downstream enforcement are reproved; salvage the legacy positive and negative composition tests |
