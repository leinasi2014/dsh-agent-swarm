...
| Official host-tool inheritance and child report/code transport | base main remains blanket default-deny; this candidate restores inherited downstream authority, `run_code`, and guarded child-scoped `report` pass-through | legacy evidence supplied the compatibility shape | The candidate denies only explicit policy/captain-only/global-report cases; official downstream guards remain decisive | `CANDIDATE / integration pending`: no second Team policy/store; scoped report bypasses Team membership lookup and returns to the official pipeline |
