...
| Directed member task (`targetMemberSessionId`) | The feature chain implements one canonical persisted target, captain-only name-to-durable-child resolution, strict scheduling, lifecycle release and redacted read projections; merged-target readback remains authoritative | legacy adds schema/domain/create/cancel/reassign/scheduler/read-surface support and negative tests | Targeted scheduling is recovered without lazy-member creation or a second authority | `REIMPLEMENT` on the current Team domain/scheduler; preserve legacy negatives as specification |
