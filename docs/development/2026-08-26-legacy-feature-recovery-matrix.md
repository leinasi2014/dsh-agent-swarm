...
| Directed member task (`targetMemberSessionId`) | The feature chain implements one canonical persisted target, captain-only name-to-durable-child resolution, strict scheduling, lifecycle release and redacted read projections; current-domain readback remains authoritative | legacy adds schema/domain/create/cancel/reassign/scheduler/read-surface support and negative tests | Targeted scheduling is recovered without lazy-member creation or a second authority | `REIMPLEMENT` on the current Team domain/scheduler; preserve legacy negatives as specification |
| Model interrupt evidence admission and supervision discipline | The current-authority implementation separates the authenticated Human-control interrupt path from the model tool path. A model request may interrupt only when the Host verifies the exact live member Agent's current turn/step has an unsettled `tool/call` past that official tool's declared `ToolDefinition.timeoutMs`; admission, real composition and restart-suffix tests cover the boundary. | legacy supplied the Host-evidence shape, `TEAM_INTERRUPT_EVIDENCE_REQUIRED`, and supervision-prompt direction | Reimplemented on the current runtime without the rejected legacy authority. Silence, thinking duration, wait count and missing Git progress are never evidence; no declared timeout grants model-interrupt authority; `firstLiveSeq` excludes pre-restart history. | `REIMPLEMENTED`: keep the model and authenticated-Human paths separate. An interrupt preserves inbox/task/roster and wakeup provides recovery; promotion evidence is the accepted Git candidate and authoritative-main read-back. |
...
8. Model interrupt admission uses only Host evidence: exact-live-current-turn/step unsettled-call proof, the official declared tool timeout, restart-suffix isolation and recovery. Do not use fixed 60-second/10-minute, silence, thinking, wait-count or Git-progress heuristics.
