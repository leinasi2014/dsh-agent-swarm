...
6. **D1 修复（#52）的回归面**：waking 确认改为仅认领形态后——pending-only 折叠语义、claim grace 与 idle 边沿重扫的收敛性、quiet 路径不变性、成员→船长唤醒报告路径的对称性、5 处旧断言迁移的等价性。（更正 2026-08-21：本条起草时 grace 为 2s，PR#56 已放宽至 5s——审查按 5s 实现。）
