...
describe('dedicated Captain topology', () => {
  ...
  it('keeps the main Session outside the Team and binds a continuable Captain child on the configured route', async () => {
    ...
    // Omitted explicit route uses the plugin configuration.
  ...
  it('rejects an unknown explicit Captain provider before creating a Team', async () => {
    sandbox = await mkdtemp(join(tmpdir(), 'dsh-dedicated-captain-misroute-'))
    mounted = await mountNodeComposition(sandbox, { captainLlmProvider: 'mock', captainModel: 'mock' })
    const rejected = await mounted.ctx.tools.execute({
      signal: SIGNAL, callId: ToolCallId('managed-unknown-route'), name: 'agent_swarm_create_managed',
      arguments: { name: 'Managed Team', description: 'Reject before effects.', captain_llm_provider: 'unknown-provider', captain_model: 'unknown-model' },
      agent: mounted.lead,
    })
  ...
  it('exposes explicit Captain routing while retaining the Main Brain observation boundary', async () => {
    ...
    expect(Object.hasOwn(properties, 'captain_llm_provider')).toBe(true)
    expect(Object.hasOwn(properties, 'captain_model')).toBe(true)
    expect(Object.hasOwn(properties, 'captain_reasoning_effort')).toBe(true)
