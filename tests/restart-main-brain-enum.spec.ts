...
describe('restart recovery of Main Brain → Captain → Team read-only enumeration/binding', () => {
  ...
  it('rebuilds ownedChildren from official Session persistence and restores enumeration/binding', async () => {
    ...
    try {
      ...
      // A PLAIN continuable child of the same Main Brain root that is NOT a Team
      // captain (no authoritative Team aggregate names it as captainSessionId) must
      // never be resurrected as a managed captain after restart. It persists with
      // the same parentSession edge that would fool the un-refined recovery.
      const siblingId = SessionId('restart-enum-plain-sibling')
      await first.ctx.subagents.startContinuable({
        provider: 'spawn',
        label: 'restart-enum-plain-sibling',
        childId: siblingId,
        request: {
          prompt: [{ type: 'text', text: 'Plain sibling subagent, owns no Team.' }],
          parent: leadA,
          maxDepth: 1,
        },
        signal: SIGNAL,
      ...
      // persisted Session headers (parentSession + authoritative Team aggregate) at
      // start. Without the fix this is [] and every read below is empty / NOT_FOUND.
      const managed = second.ctx.agentSwarm.managedCaptainSessionsOf(ROOT)
      expect(managed).toContain(captainId)
      // Negative: the unrelated plain sibling (same root, persisted parentSession →
      // ROOT, but no Team aggregate names it as captain) must NOT be resurrected as a
      // managed captain by the restart recovery.
      expect(managed).not.toContain(siblingId)

      // R1 enumeration (what feeds the Team right column): the Main Brain is
      // 'main-brain' and enumerates ONLY its real Captain-owned Team (was "多个队长·0").
      ...
      expect(enumerated.teams.map(team => team.captainSessionId)).toEqual([captainId])
