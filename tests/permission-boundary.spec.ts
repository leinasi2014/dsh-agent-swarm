...
describe('tiered allow/ask/deny decision model (pure)', () => {
  it('inherits the official downstream decision for unlisted host tools, while reserving global report', () => {
    ...
    const historicalAllow = DEFAULT_TOOL_POLICY.allow!.slice(0, -publicTools.length - requestTools.length - goalTools.length - postTools.length - maintenanceTools.length)
    ...
    expect(DEFAULT_TOOL_POLICY.allow!.slice(historicalAllow.length)).toEqual([...publicTools, ...requestTools, ...goalTools, ...postTools, ...maintenanceTools])
    for (const name of [...publicTools, ...postTools, ...maintenanceTools]) {
    ...
    for (const name of goalTools) {
      expect(DEFAULT_TOOL_POLICY.allow).toContain(name)
      expect(decideToolPermission(DEFAULT_TOOL_POLICY, name, captainTurn())).toBe('allow')
      expect(decideToolPermission({ allow: [name] }, name, { ...captainTurn(), callerRole: 'delegated-member' }))
        .toBe(name === 'agent_swarm_get_goal' ? 'allow' : 'deny')
      if (name !== 'agent_swarm_get_goal') expect(memberToolPolicyFilter({ allow: [name] }).deny).toContain(name)
    }
