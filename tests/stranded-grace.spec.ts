...
it('keeps phase, owner and creation time through fresh-idle grace, retries at its deadline, and leaves a cold owner untouched', async () => {
  ...
  try {
    ...
    const followup = vi.spyOn(ctx.subagents as unknown as HostPromptDeliverer, deliverSubagentPrompt).mockImplementation(async () => { live = false; return 'grace-followup' as never })
