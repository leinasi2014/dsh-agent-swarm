...
describe.skipIf(!windows)('Windows candidate process boundary (issue #126)', () => {
  ...
  it('rejects the controller SID before attempting account logon', () => {
    const systemWhoami = join(process.env.SystemRoot ?? 'C:\\Windows', 'System32', 'whoami.exe')
    const identity = spawnSync(systemWhoami, ['/user', '/fo', 'csv', '/nh'], { encoding: 'utf8', windowsHide: true })
