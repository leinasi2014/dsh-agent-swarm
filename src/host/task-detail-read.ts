  ...
  const value = {
    ...
    task: { ...(version === 2 ? taskReadSummaryV2(task, team, bindingRoot, names, observedAt) : taskReadSummary(task, bindingRoot, names)),
      ...(version === 2 ? { ...taskEventFacts(task),
        ...(task.cancellation === undefined ? {} : { cancellation: { reason: task.cancellation.reason,
          actorSessionId: task.cancellation.actorSessionId, at: task.cancellation.at,
          ...(task.cancellation.attemptId === undefined ? {} : { attemptId: task.cancellation.attemptId }) } }),
