...
export function ownsFreshV2InitialModelDispatch(input: {
  ...
  readonly permits: Map<string, FreshV2ModelPermit>
  readonly retiredSignals: WeakSet<AbortSignal>
  readonly witness: FreshV2WitnessCapability
}, options: GenerateOptions): boolean {
  const hasPermit = ownsFreshV2ModelPermit(input.permits, input.retiredSignals, options, 'initial dispatch')
  ...
  try {
    ...
    if (hasPermit) retireFreshV2ModelPermit(input.permits, input.retiredSignals, options.sessionId)
    throw error
  }
}
