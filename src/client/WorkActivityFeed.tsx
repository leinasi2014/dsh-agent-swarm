...
export function WorkActivityFeed({ work, teamId, taskId, openTask, t }: {
  ...
  return <section data-work-activity aria-label={t('work.activity')} aria-busy={state.loading}>
    ...
    {page === undefined ? <p role="status">{t(state.loading ? 'loading' : 'error')}</p> : <small data-work-retained>{t('work.range', { from: page.retainedFromSequence, through: page.throughSequence, first: visible[0]?.sequence ?? 0, last: visible.at(-1)?.sequence ?? 0 })}</small>}
    ...
    {visible.map(entry => {
      ...
      const actor = entry.actor.kind === 'local-operator' ? t('public.operator') : `${entry.actor.kind === 'main' ? `${t('work.main')} · ` : ''}${entry.actor.sessionId}`
      return <article key={`${state.selection!.key}:${entry.id}`} data-work-event={entry.id} data-work-kind={entry.kind}>
        ...
        {entry.assigneeSessionId === undefined ? null : <p>{t('work.assignee')}: {entry.assigneeSessionId}</p>}
        ...
        {entry.workRequestId === undefined ? null : <details data-work-request={entry.workRequestId}><summary>{t('work.request')} · {entry.workRequestId}</summary>
