export const TEAM_DASHBOARD_NS = 'swarm.team-dashboard' as const

export const en = {
  'action.open': 'Team',
  'action.toolDetails': 'Tool details',
  'action.toolDetailsSelected': 'Tool details selected.',
  'action.toolDetailsUnavailableNow': 'Tool details are temporarily unavailable.',
  'title': 'Agent Team',
  'close': 'Close Team details',
  'description': 'Read-only local Team status for this Captain Session.',
  'loading': 'Loading the authoritative Team projection…',
  'reconnecting': 'Reconnecting to the Team projection…',
  'stale': 'Showing the last complete projection while reconnecting.',
  'error': 'Team status is unavailable.',
  'retry': 'Retry',
  'refresh': 'Refresh',
  'openChat': 'Open Captain Chat',
  'openingChat': 'Verifying Captain…',
  'tabs.label': 'Team views',
  'tabs.overview': 'Overview',
  'tabs.members': 'Members',
  'tabs.memory': 'Memory',
  'tabs.work': 'Work',
  'tabs.diagnostics': 'Details',
  'team': 'Team',
  'members': 'Members',
  'memory': 'Memory',
  'memoryOwner': 'Owner',
  'memoryAuthor': 'Author',
  'memoryEvidence': 'Evidence',
  'memoryTruncated': 'Showing the newest bounded memory records.',
  'tasks': 'Tasks',
  'attempts': 'Attempts',
  'budget': 'Budget',
  'interactions': 'Pending interactions',
  'capabilities': 'Capabilities',
  'empty': 'None',
  'usedTokens': 'Tokens used',
  'usedRequests': 'Requests used',
  'usedRetries': 'Retries used',
  'revision': 'Revision',
  'status': 'Status',
  'updated': 'Updated',
  'trustDetails': 'Access and capability details',
  'trust': 'Local single-user, target-bound read access; not a human principal.',
  'memberRole': 'Role',
  'sessionId': 'Session ID',
  'runtimeProvider': 'Subagent runtime Provider',
  'llmProvider': 'LLM Provider',
  'model': 'Model',
  'modelSource': 'Model selection source',
  'deniedTools': 'Creation-time denied tools',
  'assignedSkills': 'Assigned Skills',
  'dynamicTaskTools': 'Per-task dynamic tool policy',
  'unsupported': 'Not supported by the current DSH continuation seam',
  'unresolved': 'Unresolved',
  'inherited': 'Inherited',
  'noneDeclared': 'None declared',
  'truncated': 'truncated',
  'available': 'available',
  'unavailable': 'unavailable',
  'owner': 'Owner: {name}',
  'blocked': 'Blocked by {count}',
  'generation': 'Generation {generation}',
  'target': 'Target {kind}',
  'enum.active': 'Active',
  'enum.archived': 'Archived',
  'enum.provisioning': 'Provisioning',
  'enum.failed': 'Failed',
  'enum.removed': 'Removed',
  'enum.pending': 'Pending',
  'enum.in_progress': 'In progress',
  'enum.submitted': 'Submitted',
  'enum.verifying': 'Verifying',
  'enum.completed': 'Completed',
  'enum.cancelled': 'Cancelled',
  'enum.running': 'Running',
  'enum.accepted': 'Accepted',
  'enum.rejected': 'Rejected',
  'enum.stale': 'Stale',
  'enum.acknowledged': 'Acknowledged',
  'enum.captain': 'Captain',
  'enum.team': 'Team',
  'enum.member': 'Member',
  'enum.task': 'Task',
  'enum.decision': 'Decision',
  'enum.lesson': 'Lesson',
  'enum.context': 'Context',
  'enum.explicit': 'Explicit',
  'enum.member-default': 'Member default',
  'enum.captain-inherited': 'Inherited from Captain',
  'enum.unresolved': 'Unresolved',
} as const

export const zh: Record<keyof typeof en, string> = {
  'action.open': '团队',
  'action.toolDetails': '工具详情',
  'action.toolDetailsSelected': '已切换到工具详情。',
  'action.toolDetailsUnavailableNow': '工具详情暂时不可用。',
  'title': '智能体团队',
  'close': '关闭团队详情',
  'description': '当前 Captain 会话的本机只读团队状态。',
  'loading': '正在读取权威团队投影…',
  'reconnecting': '正在重新连接团队投影…',
  'stale': '重新连接期间显示最后一份完整投影。',
  'error': '团队状态暂不可用。',
  'retry': '重试',
  'refresh': '刷新',
  'openChat': '打开 Captain 对话',
  'openingChat': '正在验证 Captain…',
  'tabs.label': '团队视图',
  'tabs.overview': '概览',
  'tabs.members': '成员',
  'tabs.memory': '记忆',
  'tabs.work': '任务',
  'tabs.diagnostics': '详情',
  'team': '团队',
  'members': '成员',
  'memory': '记忆',
  'memoryOwner': '归属',
  'memoryAuthor': '记录者',
  'memoryEvidence': '证据',
  'memoryTruncated': '当前仅显示最新的有界记忆记录。',
  'tasks': '任务',
  'attempts': '尝试',
  'budget': '预算',
  'interactions': '待处理交互',
  'capabilities': '能力',
  'empty': '无',
  'usedTokens': '已用 Token',
  'usedRequests': '已用请求',
  'usedRetries': '已用重试',
  'revision': '修订',
  'status': '状态',
  'updated': '更新时间',
  'trustDetails': '访问与能力说明',
  'trust': '本机单用户、目标绑定的只读访问；不代表人类主体身份。',
  'memberRole': '职责',
  'sessionId': '会话 ID',
  'runtimeProvider': '子智能体运行 Provider',
  'llmProvider': 'LLM Provider',
  'model': '模型',
  'modelSource': '模型选择来源',
  'deniedTools': '创建时禁用工具',
  'assignedSkills': '已指派 Skill',
  'dynamicTaskTools': '每任务动态工具策略',
  'unsupported': '当前 DSH 延续会话接口不支持',
  'unresolved': '未解析',
  'inherited': '继承',
  'noneDeclared': '未声明',
  'truncated': '已截断',
  'available': '可用',
  'unavailable': '不可用',
  'owner': '负责人：{name}',
  'blocked': '被 {count} 项阻塞',
  'generation': '第 {generation} 代',
  'target': '目标：{kind}',
  'enum.active': '活跃',
  'enum.archived': '已归档',
  'enum.provisioning': '配置中',
  'enum.failed': '失败',
  'enum.removed': '已移除',
  'enum.pending': '待处理',
  'enum.in_progress': '进行中',
  'enum.submitted': '已提交',
  'enum.verifying': '验证中',
  'enum.completed': '已完成',
  'enum.cancelled': '已取消',
  'enum.running': '运行中',
  'enum.accepted': '已接受',
  'enum.rejected': '已拒绝',
  'enum.stale': '已过期',
  'enum.acknowledged': '已确认',
  'enum.captain': '队长',
  'enum.team': '团队',
  'enum.member': '成员',
  'enum.task': '任务',
  'enum.decision': '决策',
  'enum.lesson': '经验',
  'enum.context': '上下文',
  'enum.explicit': '明确指定',
  'enum.member-default': '成员默认值',
  'enum.captain-inherited': '继承 Captain',
  'enum.unresolved': '未解析',
}

export type TeamDashboardKey = keyof typeof en
