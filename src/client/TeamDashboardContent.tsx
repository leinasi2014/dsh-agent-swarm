...
function Status({ state, t }: { state: TeamDashboardState; t: TranslateNS<typeof TEAM_DASHBOARD_NS> }) {
  ...
  return <div role={failed ? 'alert' : 'status'} aria-live="polite" style={{ ...row, justifyContent: 'flex-start' }}><StateDot state={failed ? 'warning' : 'ongoing'} /><span>{label}</span>{state.error === undefined ? null : <span style={{ minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={`${state.error.code}: ${state.error.message}`}><code>{state.error.code}</code><small style={muted}>: {state.error.message}</small></span>}</div>
}
