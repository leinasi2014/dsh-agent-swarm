...
export interface TeamPluginSettings {
  enabled?: boolean
  communicationIntensity?: 'quiet' | 'balanced' | 'active'
...
export const teamSkillSettingsEn = {
  ...
  captainRoute: 'Default Captain model route', memberRoute: 'Default member model route', inheritRoute: 'Use the current creator model when no route is specified', routesLoading: 'Loading configured DSH model routes…', routesUnavailable: 'No configured model route is available. Add one in DSH Model settings.',
  memberProvider: 'Member runtime Provider', maxMembers: 'Maximum members per Team', maxTasks: 'Maximum tasks per Team', memberDepth: 'Member subagent depth', pendingMessages: 'Pending messages per member', identityPolicy: 'Identity and avatar policy', identityPolicyValue: 'The Captain assigns only the profession and responsibilities. Members choose their own name, personality and biography. Save and read back the four text fields before creating a preferred 32x32 pixel avatar: people, animals, objects or abstract designs, with coordinated colors.',
  communicationIntensity: 'Default communication intensity', communicationQuiet: 'Quiet', communicationBalanced: 'Balanced', communicationActive: 'Active', communicationHint: 'Quiet / Balanced / Active allow each member 1 / 4 / 12 proactive peer wakeups per minute. Excess messages queue quietly; no resend is needed. The first valid reply is exempt from this allowance. Captain-to-member and member-to-Captain communication and task submissions are exempt.',
...
export const teamSkillSettingsZh: Record<keyof typeof teamSkillSettingsEn, string> = {
  ...
  captainRoute: '队长默认模型路由', memberRoute: '队员默认模型路由', inheritRoute: '未指定时继承发起者当前模型', routesLoading: '正在读取 DSH 已配置的模型路由…', routesUnavailable: '尚无可用模型路由；请先在 DSH 的“模型”设置中配置。',
  memberProvider: '队员运行 Provider', maxMembers: '每个团队最多成员数', maxTasks: '每个团队最多任务数', memberDepth: '队员子智能体层级', pendingMessages: '每名队员待处理消息上限', identityPolicy: '身份与头像策略', identityPolicyValue: '队长只指定职业与职责；成员自行决定姓名、性格和简介。先保存并读回四项文字资料，最后按喜好生成 32×32 点阵头像：人物、动物、物品或抽象图案均可，配色协调。',
  communicationIntensity: '默认通信强度', communicationQuiet: '安静', communicationBalanced: '适中', communicationActive: '积极', communicationHint: '安静 / 适中 / 积极：每名成员每分钟分别可主动唤醒同伴 1 / 4 / 12 次。超额消息安静排队，无需重发；首次合法回复不受该额度限制。队长上下行通信和任务提交不受限。',
...
interface Draft {
  readonly communicationIntensity: 'quiet' | 'balanced' | 'active'
...
function Overview(props: EditorProps & { readonly routes: LoadState<TeamModelRoute> }) {
  const communicationHintId = useId()
  ...
  return <section><div style={{ ...layout.panel, marginBottom: 14 }}><Toggle label={props.t('enabled')} checked={props.draft.enabled} disabled={!props.editable} onChange={value => { props.onEdit('enabled', value) }} /></div><div style={layout.grid}>
    ...
    <label style={layout.field}>{props.t('communicationIntensity')}<select aria-label={props.t('communicationIntensity')} aria-describedby={communicationHintId} style={layout.input} value={props.draft.communicationIntensity} disabled={!props.editable} onChange={event => { props.onEdit('communicationIntensity', event.target.value as Draft['communicationIntensity']) }}><option value="quiet">{props.t('communicationQuiet')}</option><option value="balanced">{props.t('communicationBalanced')}</option><option value="active">{props.t('communicationActive')}</option></select><span id={communicationHintId} style={layout.hint}>{props.t('communicationHint')}</span></label>
