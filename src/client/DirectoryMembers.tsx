...
export function DirectoryMembers({ chat, dashboard, onTask, navigation, t }: { chat: PublicChatController; dashboard?: TeamDashboardState; onTask?: ((id: string) => void) | undefined; navigation?: TeamNavigationCallbacks; t: T }) {
  ...
  const [contactOpening, setContactOpening] = useState<string>(), [contactFailure, setContactFailure] = useState<{ member: string; reason: string }>()
  const contactAttempt = useRef(0)
  ...
  // The contact target is re-resolved from the freshest roster projection on every render, so a click navigates by precise roster name and Session id, never a hand-built URL.
  const freshMember = assets?.members.find(row => row.sessionId === memberId && row.phase === 'active')
  const isFreshCaptain = assets !== undefined && captain?.captainSessionId === memberId && entry?.role === 'captain'
  const contactEnabled = navigation !== undefined && (isFreshCaptain || freshMember !== undefined)
  ...
  // A member switch, Team switch, or profile close invalidates any in-flight contact outcome; late results must not leak into a new profile.
  useEffect(() => { contactAttempt.current += 1; setContactOpening(undefined); setContactFailure(undefined) }, [memberId, state.selection?.key])
  const startContact = (action: () => Promise<void>): void => {
    if (memberId === undefined) return
    const attempt = ++contactAttempt.current, member = memberId
    setContactOpening(member); setContactFailure(undefined)
    void action()
      .then(() => { if (contactAttempt.current === attempt) close(false) })
      .catch(error => { if (contactAttempt.current === attempt) setContactFailure({ member, reason: error instanceof Error ? error.message : t('error') }) })
      .finally(() => { if (contactAttempt.current === attempt) setContactOpening(undefined) })
  }
  return <div ref={root} className="swarm-directory" data-swarm-directory onKeyDown={event => { if (event.key === 'Escape' && memberId !== undefined) { event.preventDefault(); event.stopPropagation(); close(true) } }} onFocusCapture={() => { if (!state.directoryLoading) void chat.refreshDirectory() }}>
    ...
    <div className="swarm-directory__grid" aria-busy={state.directoryLoading}>{state.directory?.entries.map(row => {
      ...
      // The whole card opens the profile; the fixed profile header is the only contact entry into the official Chat.
      return <button type="button" key={row.memberId} data-directory-member={row.memberId} aria-label={`${row.label} · ${row.name}`} aria-haspopup="dialog" aria-expanded={memberId === row.memberId} title={`${row.label} · ${row.responsibility}`} onClick={profile}>{identity}</button>
    })}</div>
    {state.directory?.entries.length === 0 ? <p>{t('directory.empty')}</p> : null}
    {entry === undefined ? null : <MemberProfileSurface key={entry.memberId} anchorRef={trigger} rootRef={root} close={close} title={entry.label}>{dismiss => <MemberProfileContent tab={tab} onTabChange={setTab} entry={entry} result={result} resultObservedAt={assets?.observedAt} close={dismiss} onTask={onTask === undefined ? undefined : id => { close(false); onTask(id) }} t={t} contact={{
      pending: contactOpening === entry.memberId,
      reason: contactFailure?.member === entry.memberId ? contactFailure.reason : undefined,
      disabledReason: contactEnabled ? undefined : t('detail.contactDisabled'),
      onStart: () => { if (navigation === undefined) return; if (isFreshCaptain) startContact(navigation.openCaptain); else if (freshMember !== undefined) startContact(() => navigation.openMember(freshMember.name, entry.memberId)) },
    }} />}
